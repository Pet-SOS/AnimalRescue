export enum RequestFilterOperators {
  ALL = 'all',
  EQ = 'eq',
  CONTAINS = 'contains',
}
...
export const prepareRequestParams = (requestParams?: IRequestParams) => {
  ...
  if (
    typeof requestParams?.filter != 'string' &&
    !!requestParams?.filter &&
    !!requestParams?.filter?.fieldName &&
    !!requestParams?.filter?.operator
  ) {
    params.filter = `${requestParams?.filter?.fieldName}~${requestParams?.filter?.operator}~'${requestParams?.filter?.value}'`;
  }
  if (
    !!requestParams?.sort &&
    !!requestParams?.sort?.fieldName &&
    !!requestParams?.sort?.order
  ) {
...
export const prepareReadyForAdoptionRequestParams = (
