...
function* fetchHomePageAnimalsListSaga(action: {
  type: string;
...
function* deleteAnimalSaga(
  action: ReturnType<typeof actionAdminDeleteAnimalRequest>,
) {
  try {
...
function* postAnimalSaga(
  action: ReturnType<typeof actionAdminPostAnimalRequest>,
) {
  try {
...
function* updateAnimalSaga(
  action: ReturnType<typeof actionAdminUpdateAnimalRequest>,
) {
...
export function* fetchAllLocationsSaga(
  action: ReturnType<typeof actionAdminFetchAllLocationsRequest>,
) {
  try {
...
export function* watchAdminHomePage() {
  yield takeEvery(
    getType(actionAdminHomeFetchAnimalsRequest),
    fetchHomePageAnimalsListSaga,
  );
  yield takeEvery(
    getType(actionAdminFetchAllLocationsRequest),
    fetchAllLocationsSaga,
  );
