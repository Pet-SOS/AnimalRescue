...
export const adminLocationsReducer = (
  state: IAdminLocationsSate = defaultState,
  action: AnyAction,
) => {
  switch (action.type) {
    ...
    case getType(actionAdminFetchLocationsFailure):
      ...
      return {
        ...
        locations: {
          ...state.locations,
          [loadListType]: singleLocationReducer(
            state.locations[loadListType],
            action,
          ),
        },
      };
...
const singleLocationReducer = (
  state: ILocationState = { ...DEFAULT_LOCATION_STATE },
  action: AnyAction,
) => {
  switch (action.type) {
    case getType(actionAdminFetchLocationsRequest):
      return {
        ...state,
        requestState: fetchAdminLocationRequestStateReducer(
          state.requestState,
          action,
        ),
      };
...
const locationUpdate = (
  ...
  switch (action.type) {
    case getType(actionAdminUpdateLocationSuccess):
      const list = [...(state.list.data || [])].map(location =>
        action.payload.location.id === location.id
...
const locationDelete = (
  ...
  switch (action.type) {
    case getType(actionAdminDeleteLocationSuccess):
      const list = (state.list.data || []).filter(
        location => action.payload.location.id !== location.id,
      );
      return {
        ...state,
        list: {
