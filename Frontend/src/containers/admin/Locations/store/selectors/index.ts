...
export const isLoadingAdminLocations = (state: ICustomAppState, type?: LocationsCode): boolean =>
    isLoadingRequestState(selectLocationState(state, type).requestState);


export const isLoadingRequestState = (requestState?: IRequestState) => !!requestState &&
    (ERequestStatus.NONE === requestState.status || ERequestStatus.REQUEST === requestState.status);
