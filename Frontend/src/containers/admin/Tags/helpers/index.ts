...
export const buildKindOfAnimal = (
  category: string,
  originKindOfAnimal: string,
) => {
  let targetKindOfAnimal = originKindOfAnimal;
  if (
    TagCategory.dogSize.toLocaleLowerCase() === category.toLocaleLowerCase()
  ) {
...
export const buildFilter = (
  ...
  if (!kindOfAnimal) {
    ...
    return `category~eq~'${buildCategory(
      categoryName,
      kindOfAnimal,
    )}';kindOfAnimal~eq~'${buildKindOfAnimal(categoryName, kindOfAnimal)}'`;
  }
};
const ignoreAdd = [
...
export const isLevelSupport = (targetCategory: string, kindOfAnimal?: string) =>
  !(
    FilterType.KIND_OF_ANIMAL.toLocaleLowerCase() ===
      targetCategory.toLocaleLowerCase() && !kindOfAnimal
  );
