...
export const BlogEditForm: React.FC<IBlogEditFormProps> = ({blog, onUpdate, baseUrl}) => {
    ...
    const removeLoadedImage = (id: string) => {
        setFields((prevState: IBlogFields) => {
            return {...prevState, imageIds: prevState.imageIds.filter(imageId => imageId !== id)}
        });
    };
    ...
    return (
        <div>
            ...
            {fields.imageIds.map(id => {
                return <div className="image-preview-container" key={id}>
                  <img className="image-preview" src={getImageSrc(id)} alt="Uploaded" />
            ...
            {images.map(image => {
                return <div className="image-preview-container" key={image.name}>
                  <img className="image-preview" src={URL.createObjectURL(image)} alt="Uploaded" />
                    <div className="delete-image" onClick={() => removeImage(image.name + image.lastModified) }>
                    <img src={CloseIcon} alt="delete"/>
                  </div>
                </div>
            })}
            <div className="add-image-button-container">
                <div className="add-image-button" onClick={() => fileInputRef.current.click()}>
                    ...
                    <input type="file" className="hidden" ref={fileInputRef} onChange={onImageChange} />
                </div>
