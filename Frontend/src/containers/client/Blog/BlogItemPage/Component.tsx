...
export const BlogItemPageComponent: React.FC<IPropTypes> = ({
  ...
  const { isLoading, isLoaded } = useSelector(
    () => selectBlogItem(store.getState()),
    shallowEqual,
  );
  const { status } = useSelector(
    () => selectBlogItem(store.getState()).requestState,
    shallowEqual,
  );
  ...
  const getItemDateString = (): string => {
    ...
    return `${currentItemDate.getDate()}.${
      currentItemDate.getMonth() < 9
        ? `0${currentItemDate.getMonth() + 1}`
        : currentItemDate.getMonth() + 1
    }.${currentItemDate.getFullYear()}`;
  };
  const getItemSubtitle = (): JSX.Element => {
    const dateString = getItemDateString();
    const itemTypeSubtitle =
      blogItem.data.type === BlogTypes.ARTICLE ? (
        <TI18n keyStr="blogarticle" default="Полезные советы" />
      ) : (
        <TI18n keyStr="blogstory" default="Истории успеха" />
      );
    return (
      <React.Fragment>
        {!!dateString ? <span className="item-date">{dateString}</span> : ''}
        {itemTypeSubtitle}
      </React.Fragment>
    );
  };
  ...
  return (
    <div className="blog-item-page-holder">
        ...
        {isLoaded && !isLoading && (
          <React.Fragment>
            <Banner
              ...
              imgLink={`${baseUrl}documents/${blogItem.data.imageIds[0]}/type/large`}
            />
            <div className="blog-item-content section-margin">
              <div
                className="story-body section-margin"
                dangerouslySetInnerHTML={{
                  __html: DOMPurify.sanitize(blogItem.data.body),
                }}
              />
              {blogItem.data.type === BlogTypes.ARTICLE && (
                <div className="warning">
                  ...
                  {!!phones && !!phones[0] && (
                    <strong>
                      <a className="number" href={`tel:${phones[0]}`}>
                        {' '}
                        {phones[0]}
                      </a>
                    </strong>
            ...
            {blogItem.data.type === BlogTypes.STORY &&
              !!blogItem.data.imageIds.length && (
                <div className="block-holder">
                  <Slider
                    slides={blogItem.data.imageIds.map(imgId => (
      ...
      {!!blogList &&
        !!blogList.data &&
        !!blogList.data.filter(item => item.id !== blogItem.data.id).length && (
          <BlogBlock
            title={
              blogItem.data.type === BlogTypes.STORY ? (
                <TI18n
                  keyStr="moreSuccessStories"
                  default="Еще историй успеха"
                />
              ) : (
                <TI18n
                  keyStr="moreUsefulAdvices"
                  default="Еще полезных статей"
                />
              )
            }
            data={blogList.data
              .filter(item => item.id !== blogItem.data.id)
              .slice(0, 3)}
          />
        )}
      <HelpBlock
