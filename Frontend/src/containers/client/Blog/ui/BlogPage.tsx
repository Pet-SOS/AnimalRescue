    ...
    return (
      <div className="blog-page">
        <div className="container">
          <h2 className="title">
            {' '}
            <TI18n keyStr="blogPageTitle" default={defaultText.blogPageTitle} />
          </h2>
          <ul className="box-btn">
            <li
              className={this.state.activeBtn === 'all' ? `active all` : 'all'}
              onClick={e => this.filterItemInBlog('all')}
            >
            ...
            {Object.keys(this.blogFilter).map((item, i) => (
              <li
                ...
                key={i}
                className={
                  this.state.activeBtn === item ? `active ${item}` : item
                }
              >
                {this.blogFilter[item]}
              </li>
            ))}
          </ul>
