...
export const AnimalsSlider: React.FC<IPropTypes> = ({ data, title, link }) => {
  ...
  useEffect(() => {
    const setWindowParams = () => {
      ...
      setSlidesPerView(
        window.innerWidth > 1023 ? 3 : window.innerWidth > 600 ? 2.33 : 1.2,
      );
    };
    setWindowParams();
  ...
  const getSlideElements = (): React.ReactNode[] => [
    ...data.map(animal => <AnimalCard animal={animal} />),
  ];
  return (
