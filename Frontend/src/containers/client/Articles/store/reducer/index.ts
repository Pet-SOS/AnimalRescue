...
export const articlesReducer = (state: IArticlesState = DEFAULT_ARTICLES_STATE, action: AnyAction): IArticlesState => {
  switch (action.type) {    
    case getType(actionFetchArticleListRequest):
        return {
        ...state,
        articleListRequestState: fetchArticleListStateReducer(state.articleListRequestState, action),
      }
