...
export const TagTranslation: React.FC<IPropTypes> = ({ tagId }: IPropTypes) => {
    ...
    const getTagFromStore = (str: string): any => {
        ...
        const currentTag: ITag[] = tagsList.filter(tag => tag.id?.toLowerCase() === str.toLowerCase());
      ...
      const capitalizedString = (str: string): string => {
        ...
        const newStr: string = !!currentTag && !!currentTag.length ? currentTag.trim().toLowerCase() : str.trim().toLowerCase();
